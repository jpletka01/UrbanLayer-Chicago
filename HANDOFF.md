...
Three detailed implementation plans for the Tier 2 features. **All three implemented on 2026-06-02.** Tier 2 focuses on infrastructure that must exist before monetization: intelligent token management, latency reduction, and exportable reports that provide standalone value. Implementation order: Plan 2 (smallest, extends existing patterns) → Plan 3 (medium, frontend-focused) → Plan 1 (largest, core pipeline change).

**Implementation summary (2026-06-02):**

**Plan 2 (Reduce Latency) — DONE.** Added TTLCache to 5 Socrata core queries: crime/311 (15min TTL), permits/violations/business (1hr TTL), plus 3 map-data caches. Extended TTLCache with hit/miss counters, `stats()` method, and named instances for all 17 caches across the codebase. Created shared singleton `httpx.AsyncClient` in `socrata.py` with HTTP/2 and connection pooling (20 max connections, 10 keepalive). Removed per-request client creation from `_retrieve()`, `_fetch_map_rows()`, and `/api/map-data`. Added `/api/admin/cache` endpoint showing per-cache stats and overall hit rate. Updated `requirements.txt` to `httpx[http2]`. Files: `backend/retrieval/cache.py`, `backend/retrieval/crime.py`, `backend/retrieval/three11.py`, `backend/retrieval/buildings.py`, `backend/retrieval/business.py`, `backend/retrieval/map_data.py`, `backend/retrieval/socrata.py`, `backend/main.py`, `requirements.txt`, plus name additions to 11 existing TTLCache instances.

**Plan 3 (Conversation Export) — DONE.** Client-side PDF export with lazy-loaded jspdf + html2canvas (~600KB loaded only on download click, zero initial bundle impact). Created `reportBuilder.ts` (structured report from conversation messages covering property, regulatory, incentives, neighborhood, safety, Q&A, code citations, disclaimer), `pdfExport.ts` (multi-page A4 PDF generation), and `ExportReport.tsx` (full-page overlay with scrollable preview, Download PDF / Print / Close buttons, inline-styled tables for html2canvas compatibility). Added "Report" button in workspace header (visible only when assistant context data exists and not streaming). Added `preserveDrawingBuffer: true` to Mapbox GL JS for canvas screenshot capture. Added `@media print` stylesheet. Files: `frontend/src/lib/reportBuilder.ts` (new), `frontend/src/lib/pdfExport.ts` (new), `frontend/src/components/ExportReport.tsx` (new), `frontend/src/lib/useMapboxOverlay.ts`, `frontend/src/App.tsx`, `frontend/src/index.css`, `frontend/package.json`.

**Plan 1 (Intelligent Context Management) — DONE.** Created `backend/context_manager.py` with `summarize_turn()` (pure function, no LLM call — extracts ~100-200 token summary per turn), `format_summaries_for_prompt()` (groups by location, marks different-location turns), and `detect_location_switch()` (triggers isolation instruction). Added `TurnSummary` Pydantic model to `models.py`. Schema v3 migration adds `summary_json TEXT` column to messages table (non-destructive ALTER TABLE ADD COLUMN). Refactored `synthesizer.py`: when summaries exist, builds history from summary text + last 2 user/assistant pairs (sliding window) instead of full verbatim history; location-switch detection injects explicit isolation instruction. Added `turn_summary` SSE event type to `ChatChunk`. `_event_stream` in `main.py` loads prior summaries from SQLite on each request and generates a new summary after synthesis completes. Frontend handles `turn_summary` SSE event in `useChat.ts`, stores on Message, persists to SQLite via `history.ts`. Updated `prompts.py` with summary-aware system instructions. Backward compatible: conversations without summaries fall back to full-history approach. Files: `backend/context_manager.py` (new), `backend/models.py`, `backend/db.py`, `backend/synthesizer.py`, `backend/main.py`, `backend/prompts.py`, `frontend/src/lib/types.ts`, `frontend/src/lib/useChat.ts`, `frontend/src/lib/history.ts`.
