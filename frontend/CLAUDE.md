...
| `src/components/PageHeader.tsx` | Shared header for non-chat pages (Scorecard/Explore/Pricing/About): 3-zone layout — logo left, nav centered (Analyst/Scorecard/Explore/Pricing/About with active highlight; "Analyst" → `/?analyst=1` opens the hero in chat mode), LanguageSelector + UserMenu/sign-in right. `sticky`/`maxWidthClass` props. **Discovery is inserted into nav (after Explore) only when its index has data (`coverage.mode != "none"`, fetched via cached `registryClient`) — `navItemsFor()`; self-activated on prod 2026-06-14.** Splash & workspace keep their own headers |
...
- **Routing**: `/` (splash — address-first hero via `HeroEntrance` → `/scorecard?address=`; librarian chat secondary), `/c/:id` (conversation), `/s/:shareToken` (shared read-only view), `/scorecard` (property scorecard, non-AI), `/explore` (Site Explorer, premium-gated), `/discovery` (Property Discovery workbench — free teaser/top-10 + premium full list+map+export; **LIVE, nav-linked, 25 CAs**), `/pricing` (Free vs Pro plan comparison), `/admin` (dashboard, admin-only via `ProtectedRoute`), `/about` (technical deep dive).
