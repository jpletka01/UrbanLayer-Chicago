...
| `src/discovery/` | **Property Discovery** (filter/search, `/discovery`, premium-gated; built 2026-06-13, not deployed). The FE never evaluates — it compiles panel/topic/text inputs into a `SearchRequest` and renders chips + summary **from `response.cqs`** (INV-4). Modules: `types.ts` (wire mirrors), `registryClient.ts` (cache+staleness), pure `uiCompiler`/`topicCompiler`/`summary`, `searchClient`, `chips.tsx`, `DiscoveryFilterPanel`/`DiscoveryResults`/`DiscoveryPage`. Results are **list-first** (PIN→Scorecard via `?pin=`); map markers deferred (wire returns pins only). One-tap re-issue folds the evaluated CQS back into panel state. Spec/decisions: `claude-context/property-discovery/`. Tests via **vitest** (`npm test`) |
