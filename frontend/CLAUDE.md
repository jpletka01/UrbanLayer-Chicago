...
| `src/components/PageHeader.tsx` | Shared header for non-chat pages (Scorecard/Explore/Pricing/About): logo, nav with active highlight (incl. Pricing), LanguageSelector + UserMenu/sign-in top-right. `sticky`/`maxWidthClass` props. Splash & workspace keep their own headers |
