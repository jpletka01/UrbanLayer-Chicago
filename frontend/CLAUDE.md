...
- **Map layer defaults** (`MapView.tsx`, revised 2026-06-12 map-relevance review): zoning/incentives/overlays ON when present; transit OFF (ratchets on via `hasTransitContext`); crime/311/permit dots OFF. Dots auto-enable only two-factor: `deriveFilterMode(sources)` is single-source AND `pointsAreSubject(intent)` (incident_lookup/trend_analysis/event_query) — a legal question retrieving permits as context stays quiet. Backend gates the zoning quilt + overlay layers on `_landuse_map_relevant(plan)` (land-use vocabulary or site_due_diligence workflow), not `requires_disclaimer`. **The map doesn't render at all without spatial content**: `hasSpatialMapContent()` (mapColors.ts) gates MapView in `DataMapLayout` and the Map tab in `MobileSidebarSheet`; pure code-research turns get a full-height data panel.
