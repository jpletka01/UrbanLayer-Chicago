      ...
      >
        <div
          className={`relative max-w-[85%] rounded-2xl px-4 py-3 bg-accent-muted text-text-primary transition-all
            ${onSelect ? "cursor-pointer hover:ring-1 hover:ring-dark-border-strong" : ""}
        ...
        >
          {hovered && (
            ...
            >
              {copied ? (
                <svg className="w-4 h-4 text-state-positive" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                >
                  {att.mime_type?.startsWith("image/") ? (
                    ...
                    <div className="w-full h-full bg-dark-elevated flex flex-col items-center justify-center gap-0.5">
                      ...
                      <span className="text-micro text-text-muted">PDF</span>
                    </div>
                  )}
                </button>
              ))}
