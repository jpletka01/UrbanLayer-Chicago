// Chicago's 77 community areas — the `neighborhood` region options. The region handle the
// backend index uses is `neighborhood:<id>` (see index_build.py).

export const COMMUNITY_AREAS: Record<number, string> = {
...
export const SORTED_CAS = Object.entries(COMMUNITY_AREAS)
  .sort(([, a], [, b]) => a.localeCompare(b))
  .map(([id, name]) => ({ id: Number(id), name }));

export const NEIGHBORHOOD_PREFIX = "neighborhood:";

export function caName(ref: string): string {
  const id = Number(ref.replace(NEIGHBORHOOD_PREFIX, ""));
  return COMMUNITY_AREAS[id] ?? ref;
}
