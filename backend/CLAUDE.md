...
| File | Purpose |
|------|---------|
| `main.py` | FastAPI app (~2,350 lines since the 2026-07-09 report extraction): `/chat` SSE, `/api/scorecard`, **`/api/area-stats`** (Property Profile KPI benchmarks — per-CA aggregates from `retrieval/area_stats.py`, all-None when the Discovery index is absent), **`/api/parcel-map`** (geometry layers for the Profile's module maps: `zoning_polygons_near` bbox quilt + overlay/TIF/EZ geojson reuse, `_simplify_geojson`'d + TTL-cached), `/api/report` (route + Jinja HTML render + isolated-child PDF render only — the builder lives in `report_builder.py`), `/api/report/access`, `/api/checkout` (subscription), `/api/checkout/report` (one-time $25 report purchase), `/api/webhook/stripe`, `/api/subscription`, `/api/billing/portal`, `/api/conversations/*`, `/api/admin/*` (includes `/api/admin/engagement`), `/api/events` (CSRF-exempt, fire-and-forget analytics ingestion; `_VALID_EVENT_NAMES` allowlist deliberately EXCLUDES purchase_completed/subscription_started — the Stripe webhook is their only writer), `/api/newsletter` (CSRF-exempt anonymous email capture, server-side validation), `/api/map-data`, `/api/transit-stations` |
| `report_builder.py` | **The $25 report pipeline** (extracted verbatim from main.py 2026-07-09, ~2,940 lines): `_fetch_report_data()` (data assembly), matplotlib map/chart rasters (`_generate_zoning_map/_construction_map/_comps_map/_parcel_map/_envelope_map`, `_generate_comps_chart`), and the deterministic synthesis passes — `_synthesize_opportunities_constraints()`, `_compute_land_value_range()`, `_compute_approval_pathway()`, `_compute_development_trend()`, `_build_incentive_stacking_narrative()`, `_build_envelope_summary()`, `_build_decision_box()` (all deterministic, no LLM calls) — plus `_apply_mock_overrides()` and the scorecard-shared `_comp_class_prefix()`. Coupling seam: `_fetch_report_data` lazily imports main's `_fetch_scorecard_data` + `_limited` (process-wide retrieval semaphore) at call time — see the module docstring before "fixing" that |
...
| `analytics.py` | Server-side MoM trend computation from raw Socrata rows. Owns `_normalize_permit_type` (assembler imports it; FE port in mapColors.ts). Capped fetches drop the truncated oldest month (`_map_capped_flags` in main.py feeds it); prior-count-0 → `change_pct=None` (renders "new", never "+100%") |
...
| `payments.py` | Stripe integration: subscription checkout + a la carte report checkout (`mode=payment`; PIN carried in session metadata, purchase row, and `?pin=` success/cancel URLs when known), webhook handler dispatches on session mode (subscription vs one-time) and matches purchases by `stripe_session_id`, billing portal, subscription status. **Analytics (2026-07-05):** `visitor_id` rides checkout metadata (from the FE's `getVisitorId()`); on completion the webhook writes `purchase_completed`/`subscription_started` events via `_save_money_event` — server-only, spoof-proof funnel money step |
| `rate_limit.py` | Per-user sliding window rate limiting + daily API budget cap. Applied to `/chat` only |
| `db.py` | SQLite persistence (aiosqlite, WAL, **schema v14**). Tables: conversations (user-scoped), messages, uploads, llm_calls, request_logs, users (with stripe_customer_id, stripe_subscription_id), refresh_tokens, conversation_shares, report_purchases (a la carte $25 reports, entitlement keyed on parcel PIN when known; legacy pin-less rows matched by lat/lon rounded to 4 decimals; v12 = nullable user_id tombstone), events (usage analytics — 16 client events allowlisted in main.py + 2 webhook-written money events; see frontend/CLAUDE.md tracking.ts row for the full list), **subscribers (v13: newsletter emails, UNIQUE lowercased, `add_subscriber`)**, **vouchers + voucher_redemptions (v14: early-adopter comp premium — `users.premium_until` epoch-ms grants effective premium via the `_apply_comp_premium` choke point in `auth.get_current_user`; deliberately separate from `tier` so Stripe webhooks can't clobber a grant; redeem = `db.redeem_voucher`, admin mint/grant endpoints in main.py, UI in AdminDashboard "Early Access")**. `get_engagement_stats` returns the 6-step acquisition funnel (distinct visitors/step) + per-visitor channel classification (utm_source > referrer host > direct, from visit_start attribution) |
...
| `log_safety.py` | **CR/LF escaping on every log record.** Installed on the root HANDLERS in `_startup` (handlers, not loggers — a logger filter only sees records logged directly to it, while a handler filter sees everything that propagates, including library loggers). User-controlled strings (addresses, chat messages, Stripe ids) reach `log.*` throughout `retrieval/`; without this an embedded newline forges what reads as an extra authentic log line. Escapes rather than strips, leaves non-strings (esp. exceptions — traceback rendering depends on them) untouched, is idempotent, never drops a record. This is why the ~27 CodeQL `py/log-injection` alerts are dismissed rather than fixed per-site: static dataflow can't see a runtime filter. Tests: `test_log_safety.py` |
...
├── zoning.py               # ArcGIS zoning point lookup + polygon fetch + adjacent_parcel_zoning() + zoning_polygons_near() (parcel-scale bbox quilt for the Profile's zoning map)
├── area_stats.py           # Per-CA benchmark aggregates for the Property Profile KPI band: ONE daily full pass over the Discovery index (all 77 CAs at once, run_in_executor, ~s) → median AV + median MARKET-VALUE/land-ft² (AV ÷ class assessment level — raw AV/ft² mixed 10%/25% levels and INVERTED the comparison for commercial subjects, 2026-07-07 audit D3; exempt excluded) + per-land-use medians (n≥20 floor). Every median ships its n (index land_sqft is sparse — Uptown's $/ft² median rests on 189/4,905; FE suppresses under MIN_BENCHMARK_N=50 and for condo units 299/399). Failed scan caches 5 min, not 24 h. On-demand by design — no index rebuild needed to ship; absent index ⇒ None ⇒ UI renders no benchmark. NO area eff-tax-rate (index holds no bills — FE uses the deterministic class-norm constant instead)
├── zoning_definitions.py   # Deterministic zone class lookup table (~50 entries). FAR, height (honest frontage ranges; RM-6+/M/D carry digit-free "no cap" strings that parse to numeric None), min lot size (R-only) + min-lot-per-unit dash tables (R/BC/D), uses, code sections from Title 17. NO lot coverage — not a Title-17 standard. Every value regression-tested against the ingested ordinance corpus by test_zoning_ordinance_parity.py (2026-07-06 audit — hand-typed heights had drifted into fiction). Used by PDF report + serialized as `zone_definition` in /api/scorecard (frontend ZoningCard; contract test test_zone_definition_contract.py). Fallback chain: exact → prefix → PD/PMD → unknown
...
├── property/               # Orchestrator: parcels (GIS primary, Socrata fallback) → PIN → [characteristics, assessments, sales, tax, parcel_geometry] parallel + a CONDITIONAL phase-2 (building_facts fallbacks, only when x54s left gaps; skipped for vacant 1xx). ⚠️ x54s keeps the last regression-class row FOREVER — `_chars_describe_prior_structure` drops a chars row asserting building facts when the parcel is currently classed vacant 1xx (622 W Deming: a 2013 class-207 coach-house row on a class-100 lot read as a current 4-room building, 2026-07-07). sales.py also has nearby_comparable_sales() (3-hop: Parcel Universe → Sales → Characteristics; cos(lat) bbox + haversine-filtered to the disclosed radius, three-state sale_type — unknown bldg ≠ LAND, price-less comps dropped from n)
