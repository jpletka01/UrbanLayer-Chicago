...
| Assessor - Parcel Addresses | `3723-97qp` | pin, pin10, year, **prop_address_full** (e.g. `481 W DEMING PL`) | **Second authoritative address→PIN source** (1.86M rows, current to 2026) — **wired in behind a flag (`assessor_address_resolution_enabled`, default OFF):** `assessor_address_to_pin()` (`retrieval/property/parcel_addresses.py`), step 3.5 of `_resolve_location`. Covers `78yw-iddh` holes (e.g. `481 W Deming Pl` → unique `14283190070000`, absent from Address Points). No coords → backfill centroid via `pabr-t5kh`. Match by re-parsing `prop_address_full` (a raw `like '481 W DEMING%'` also matches `4810…`); require a single distinct PIN. See `audits/2026-06-22_gis-parcel-resolution-fix.md` |
| ccgisdata - Parcel 2021 | `77tz-riq7` | pin10, **the_geom** (MultiPolygon), latitude, longitude, assessorbldgclass | **Parcel geometry via a *working* Socrata endpoint** (1.43M parcels) — *not yet wired in.* `within_circle(the_geom,lat,lon,m)` works; server-side `intersects()` does NOT (empty even for a polygon's own centroid) → do shapely point-in-polygon locally on `within_circle` candidates. 2021 vintage (boundaries are stable). Would restore the report envelope map + replace the broken GIS MapServer. See `audits/2026-06-22_gis-parcel-resolution-fix.md` |
