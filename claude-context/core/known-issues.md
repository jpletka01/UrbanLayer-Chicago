...
**Discovery index expansion is RUNTIME-bounded on the 8 GB box, not build-bounded** (build fixed 2026-06-14): the build OOM is **resolved** — the builder is now memory-bounded by construction (per-CA `upsert_parcels` ingest + a streaming `finalize_index` that recomputes value_percentile/populated_fields/recipe_counts over the SQLite index in chunks) and runs **off the live backend** in an ephemeral `docker compose run --rm --no-deps backend …` container (own cgroup, shared `backend_data` volume). `--all`/`--refresh` no longer hold the whole set in memory. The fix also corrected a coupled bug: the old `write_index` **clobbered** the `meta` row (community_areas/populated_fields/recipe_counts) to the last batch, so coverage could only grow by rebuilding everything at once; `finalize_index` now recomputes meta cumulatively (union of CAs), so `--community-areas <batch>` correctly *adds*. **The remaining ceiling is runtime:** `parcel_source.ensure_loaded` → `read_index` loads the ENTIRE index into RAM (~2 KB/parcel), so ~1.8M ≈ ~3.7 GB resident on top of ~600 MB ML models. Expand in batches, measure backend RSS (`docker stats`), and **stop when RSS ≈ 5.5 GB** (decision: hard-cap at 8 GB, no box bump — full 77 CAs is not expected to fit, an accepted limit). Index persists on `backend/data` (`settings.discovery_index_path`); monthly `--refresh` timer (now `run --rm`) auto-follows the current CA footprint. See `deploy/README.md` for the expand procedure + stop rule.
