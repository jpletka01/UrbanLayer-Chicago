...
**Reranker (`bge-reranker-v2-m3`) hangs `semantic_search` → DISABLED in prod** (found 2026-06-16): a single reranked `semantic_search` takes **>40–60s** on the prod box (CPX32, 4 vCPU), so the report's `extract_zoning_standards` — which fires **5 reranked searches in parallel** — hangs `_fetch_report_data` past the nginx 180s ceiling → **`/api/report` 504**. Report-only path, so scorecard/chat stayed up; this was the *actual* root cause of the "report never generates" incident (NOT the OOM). **Mitigation: `RERANKER_ENABLED=false` in `docker-compose.prod.yml`** — with it off, `semantic_search` is 0.2–1.4s and reports render in ~10s. **The naive "set torch threads=1" fix was DISPROVEN** (capping `torch.set_num_threads(1)` made it *worse* — it stripped intra-op parallelism without removing the 5-way serialization; re-enable attempt `c3410ba` reverted in `a465014`). **PROFILED & FIXED on `fix/report-oom` (2026-06-18):** the real cause is **unbounded rerank concurrency** (the 5 parallel searches each dispatch a torch `predict()` to the shared executor and serialize/thrash) **compounded by a 3× oversized batch** (60 pairs reranked to return 3 — `predict(4 pairs)`=0.3s, so per-pair cost was never the issue, batch size + concurrency were). Settled by a native run where swap was physically impossible yet the stall still reproduced at **0.99× the serial floor** → CPU-serialization, not swap. Fix = rerank only the top `reranker_candidate_count` (20), route every `predict()` through a **single-worker executor** (`_get_rerank_executor()`), and set `reranker_torch_threads` (default 2, configurable). The 60→20 cut is the dominant, core-independent lever (native 5-way **35.7s→12.4s**; 240 tests green). **Flag stays `RERANKER_ENABLED=false` until one report run on a genuine 4-vCPU/8GB Linux box confirms wall-time well under the 180s ceiling and picks `reranker_torch_threads` (2 vs 4) — NOT yet closed.** While off, chat loses the 20% rerank blend and zoning extraction sees weaker context (more `low`-confidence → more table fallback). Full record: `archive/2026-06-16_report-oom-reranker.md`.
