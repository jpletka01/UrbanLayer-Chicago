...
- `guides/selected-parcel-plan.md` — **SelectedParcel unification: canonical implementation handoff. Phases 0+1 IMPLEMENTED & VERIFIED 2026-06-11 on branch `selected-parcel` (commits 7a610d6, 2b0c8b1 — not pushed, awaiting review). Phases 2 (PIN-bound purchases, db migration v11) + 3 (type enforcement) pending.** Frontend identity-state primitive (pin + confidence). Read this first (and only this) for parcel-identity work, including §8 "Implementation status & findings" for deviations discovered during 0+1.
