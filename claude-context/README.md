...
- `guides/selected-parcel-plan.md` — **SelectedParcel unification: canonical implementation handoff. Phases 0+1+2 IMPLEMENTED & VERIFIED 2026-06-11 on branch `selected-parcel` (commits 7a610d6, 2b0c8b1, 82fd399 — not pushed, awaiting review; 577 unit tests + Stripe test-mode e2e pass). Phase 3 (type enforcement + cleanup) pending; completes the plan → archive after.** Frontend identity-state primitive (pin + confidence); purchases now PIN-keyed (db schema v11). Read this first (and only this) for parcel-identity work, including §8 "Implementation status & findings" for binding deviations discovered during 0+1+2.
