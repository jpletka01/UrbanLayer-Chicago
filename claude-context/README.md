...
- `property-discovery/` — Engineering handoff spec + implementation record. **Status: SHIPPED to production in two waves, still DORMANT by design (premium-gated full experience, unlinked from nav, empty prod index → coverage "none", everything reads "coming").** Wave 1 (2026-06-13, `f192d57`) = the invariant core/compilers/evaluator + list-first FE. **Wave 2 (2026-06-14, merge `8c279c6`) = the `result.rows` workbench (PR1–PR10):** `result.rows` wire + single `_resolve` path, **32 filters** + hand-authored labels + range metadata + **6 recipes**, coverage banner + `populatedFields` (honest cold-start), scannable row-cards + infinite scroll + 3-way zero states, **deck.gl map** (`/search/pins`), **premium CSV export** (`/search/export`), query-first recipe-shelf IA, **server-enforced free teaser** (top-10 + gated flag + land-use view-only map), mobile `[List|Map]` + a11y. **Start with `10-implementation-status.md`** — read the new "The result.rows workbench (PR1–PR10)" section first (the 10 PRs, structural guarantees, design decisions incl. the 0/exempt reconcile, what remains). The real launch is gated on **PR-INDEX → PR-VAL → PR-LIVE** (blocked by the 2026-06-13 Socrata 503). Normative spec docs 00–09 unchanged (note drift: `03` still says 29 filters — live registry is 32 after PR2; `07` wire is now `{rows,total,nextOffset,gated}` + `/search/pins` + `/search/export`). Implementation: `backend/discovery/` + `frontend/src/discovery/`. **Direction: Discovery is now a strict superset of `/explore`** (has the map) → retire `/explore` after the index lands. Strategy/why: `strategy/property-discovery-filters.md`.
