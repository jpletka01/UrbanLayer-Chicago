...
3. **"Address Points neighbor-parcel mis-resolution" — investigated and RESOLVED as not a bug (2026-06-11, same day)**: the QA premise was wrong, not the resolver. The Cook County Assessor confirms pin `14283180570000` *is* 443 W Wrightwood Ave (class 391 apartment building) — Address Points resolved it correctly with authoritative confidence. The EX subject `14283190070000` is actually **481 W Deming Pl** (~210 m away, a different block face — not even adjacent); the "443 W Wrightwood = EX subject" pairing was a labeling error introduced during V6 report QA (2026-06-10) and propagated through docs/memory/test fixtures. The EX subject genuinely has *no* address point (confirmed by pin-keyed query), so by-address resolution for it correctly yields approximate/no-pin — the original R7 spec claim held all along, and the "cold-cache nondeterminism" was this designed fallback under a transient Socrata failure (query exceptions return None and are not cached; only confirmed empty results are cached 24 h). No instance of Address Points resolving an address to the wrong parcel is known. Collateral: V6 QA reports generated with `?pin=14283190070000&address=443+W+Wrightwood+Ave` used a mismatched pair, so address-keyed report sections (permits-by-address, Walk Score label) pulled data for the wrong building. QA the approximate badge via `?lat&lon` (or 481 W Deming Pl).
